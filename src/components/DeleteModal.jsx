...
import { deleteQuotation } from "../StoreRedux/quotationSlice";
...
const DeleteModal = (props) => {
  ...
  const handleDeleteClick = async () => {
    ...
    try {
      ...
      else if (props.whatdelete === "quotation") {

        const delResponse = await axios.delete(
          `${serverUrl}/api/quotation/${delId}/delete-quotation`
        );
        if (delResponse.status === 200) {
          dispatch(deleteQuotation(delId));
          console.log("click quotation delte")
  ...
  return (
    <>
      {props.showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm  z-10 flex items-center justify-center enter:ease-out duration-300">
